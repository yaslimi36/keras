...
@keras_export('keras.losses.BinaryWeightedFocalCrossentropy')
class BinaryWeightedFocalCrossentropy(LossFunctionWrapper):
  """Computes the binary weighted focal cross-entropy loss.
  ...
  - `y_pred` (predicted value): This is the model's prediction, i.e, a single
    floating-point value which either represents a
    [logit](https://en.wikipedia.org/wiki/Logit), (i.e, value in [-inf, inf]
    when `from_logits=True`) or a probability (i.e, value in `[0.0, 1.0]` when
    `from_logits=False`).
  ...
  This class also takes into account a weight balancing factor for the binary
  ...
  ```python
  model.compile(
    loss=tf.keras.losses.BinaryWeightedFocalCrossentropy(alpha=0.25, gamma=2.0,
                                                         from_logits=True),
    ....
  )
  ```
  ...
  def __init__(
    ...
    """Initializes `BinaryWeightedFocalCrossentropy` instance.
    ...
    Args:
      alpha: A weight balancing factor for class 1, default is 0.25 as mentioned
        ...
        The weight for class 0 is 1.0 - `alpha`.
      gamma: A focusing parameter used to compute the focal factor, default is
        2.0 as mentioned in Lin et al., 2018.
      from_logits: Whether to interpret `y_pred` as a tensor of
        [logit](https://en.wikipedia.org/wiki/Logit) values. By default, we
          assume that `y_pred` contains probabilities (i.e., values in [0, 1]).
      label_smoothing: Float in [0, 1]. When 0, no smoothing occurs. When > 0,
      ...
      reduction: Type of `tf.keras.losses.Reduction` to apply to
        ...
        will raise an error. Please see this custom training [tutorial](
          https://www.tensorflow.org/tutorials/distribute/custom_training) for
            more details.
      name: Name for the op. Defaults to 'binary_weighted_focal_crossentropy'.
    """
    super().__init__(
  ...
  def get_config(self):
    ...
    base_config = super(BinaryWeightedFocalCrossentropy, self).get_config()
    return dict(list(base_config.items()) + list(config.items()))
...
@keras_export('keras.metrics.binary_weighted_focal_crossentropy',
              'keras.losses.binary_weighted_focal_crossentropy')
@tf.__internal__.dispatch.add_dispatch_support
def binary_weighted_focal_crossentropy(
  ...
  """Computes the binary weighted focal crossentropy loss.
  ...
  Returns:
    Binary weighted focal crossentropy loss value.
      Shape = `[batch_size, d0, .. dN-1]`.
  """
  y_pred = tf.convert_to_tensor(y_pred)
  y_true = tf.cast(y_true, y_pred.dtype)
...
@dispatch.dispatch_for_types(binary_weighted_focal_crossentropy,
                             tf.RaggedTensor)
def _ragged_tensor_binary_weighted_focal_crossentropy(
    y_true,
    y_pred,
    alpha=0.25,
    gamma=2.0,
  ...
  When used by BinaryWeightedFocalCrossentropy() with the default reduction
  (SUM_OVER_BATCH_SIZE), the reduction averages the per batch losses over
  the number of batches.
