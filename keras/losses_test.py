...
class BinaryWeightedFocalCrossentropyTest(tf.test.TestCase):
  ...
  def test_config(self):
    obj = losses.BinaryWeightedFocalCrossentropy(
        alpha=0.1,
        gamma=1.5,
        name='bfce_0',
    )
    ...
    obj_2 = losses.BinaryWeightedFocalCrossentropy.from_config(obj.get_config())
    self.assertEqual(obj_2.name, 'bfce_0')
  ...
  def test_all_correct_unweighted(self):
    ...
    obj = losses.BinaryWeightedFocalCrossentropy(gamma=1.5)
    loss = obj(y_true, y_true)
    self.assertAlmostEqual(self.evaluate(loss), 0.0, 3)
  ...
  def test_unweighted(self):
    y_true = np.asarray([1, 0, 1, 0]).reshape([2, 2])
    y_pred = np.asarray([0.9, 0.8, 0.7, 0.2], dtype=np.float32).reshape([2, 2])
    obj = losses.BinaryWeightedFocalCrossentropy(alpha=0.4, gamma=2.0)
    loss = obj(y_true, y_pred)
  ...
  def test_scalar_weighted(self):
    y_true = np.asarray([1, 0, 1, 0]).reshape([2, 2])
    y_pred = np.asarray([0.9, 0.8, 0.7, 0.2], dtype=np.float32).reshape([2, 2])
    obj = losses.BinaryWeightedFocalCrossentropy(alpha=0.6, gamma=2.0)
    loss = obj(y_true, y_pred, sample_weight=1.23)
  ...
  def test_sample_weighted(self):
    ...
    y_pred = np.asarray([0.9, 0.8, 0.7, 0.2], dtype=np.float32).reshape([2, 2])
    sample_weight = tf.constant([1.2, 3.4], shape=(2, 1))
    obj = losses.BinaryWeightedFocalCrossentropy(alpha=0.1, gamma=2.0)
    loss = obj(y_true, y_pred, sample_weight=sample_weight)
